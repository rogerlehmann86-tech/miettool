...
let handler;
const sent=[];
let isAdmin=true,alreadySent=false;
...
const query=(name)=>{
 const q={select:()=>q,eq:()=>q,upsert:()=>q,single:()=>q,maybeSingle:()=>q,then:resolve=>Promise.resolve({data:name==='admin_reservations'?row:name==='products'?{day_price:100}:name==='rental_email_templates'?[]:name==='reservation_locations'?{pickup:{name:'A',notification_email:'a@example.com'},return:{name:'B',notification_email:'b@example.com'}}:alreadySent?{sent_at:'2026-01-01'}:null,error:null}).then(resolve)};
 return q;
};
globalThis.__testCreateClient=()=>({from:query,auth:{getUser:async()=>({data:{user:{id:'admin'}},error:null})},rpc:async()=>({data:isAdmin,error:null})});
...
globalThis.fetch=async(url,options)=>{assert.equal(url,'https://api.resend.com/emails');sent.push({payload:JSON.parse(options.body),headers:options.headers});return new Response('{}',{status:200});};
