...
let handler;
const sent=[];
let isAdmin=true,alreadySent=false,partnerData=[],directQueued=true,failMail=false;
...
test('direct confirmed rental informs sublessor without customer email and deduplicates location',async()=>{
 ...
 assert.deepEqual(sent.map(s=>s.payload.to[0]),['a@example.com','b@example.com','external@example.com']);
 assert.match(sent[2].payload.subject,/Bestätigte Vermietung/);assert.match(sent[2].payload.text,/Keine Kunden-E-Mail hinterlegt/);
});
