...
function renderProducts(){const cat=el('category').value;const items=products.filter(p=>cat==='all'||p.category===cat);el('deviceGrid').innerHTML=items.map(p=>{const q=availability[p.id]??0,ok=q>0,rate=rateFor(p),total=totalFor(p),u=rentalUnits();const tiers=p.category==='Generatoren'?`<div class="tier-note">1–4 Tage ${money(p.day_price)}/Tag · ab 5 ${money(p.tier5)}/Tag · ab 20 ${money(p.tier20)}/Tag</div>`:'';const long=p.long_term?'<div class="longterm">Langzeitmiete: spezielle Konditionen auf Anfrage</div>':'';const unavailableHelp=!ok?`<div class="customer-unavailable-help"><span>Der gewählte Zeitraum ist nicht frei.</span><button type="button" class="btn calendar-btn" onclick="openAvailabilityCalendar('${p.id}')">Belegung anzeigen</button></div>`:'';return `<article class="card"><div class="device-image">${p.image_url?`<img src="${p.image_url}" alt="${p.name}" loading="lazy" onerror="this.parentElement.innerHTML='<div class=\\'device-placeholder\\'>⚙️</div>'">`:'<div class="device-placeholder">⚙️</div>'}</div><div class="card-body"><div class="category-label">${p.category}</div><h3>${p.name}</h3><div class="meta">${p.subtitle||''}</div><div class="availability ${ok?'available':'unavailable'}"><span class="dot"></span>${ok?(q>1?`${q} Stück verfügbar`:'verfügbar'):'im Zeitraum belegt'}</div>${unavailableHelp}${pickupLocationInfo(p.id)}${tiers}${long}<div class="card-footer"><div class="price"><strong>${money(total)}</strong><span>${unitsLabel(u)} · Tarif ${money(rate)}/Tag</span></div><button class="btn primary" ${ok?'':'disabled'} onclick="openBooking('${p.id}')">Anfragen</button></div></div></article>`}).join('')||'<div class="panel">Keine Geräte in dieser Kategorie.</div>';syncTabs()}
function pickupLocationInfo(productId){
  const locations=rentalLocations.filter(l=>String(l.product_id)===String(productId)&&l.can_pickup);
  if(!locations.length)return '';
  return `<details class="pickup-info"><summary>Abholstandort beachten${locations.length>1?' · mehrere Optionen':''}</summary><div class="pickup-info-body"><p>Bitte beachten Sie, wo dieses Gerät abgeholt werden muss.</p>${locations.map(l=>`<div class="pickup-place"><strong>${locationEscape(l.name)}</strong>${l.address?`<div>${locationEscape(l.address)}</div>`:''}${l.instructions?`<p>${locationEscape(l.instructions)}</p>`:''}</div>`).join('')}${locations.length>1?'<p>Den gewünschten Abholstandort wählen Sie bei der Anfrage.</p>':''}</div></details>`;
}
function showRequestResult(emailOk,email){
  let dialog=el('requestResultDialog');
  if(!dialog){
    dialog=document.createElement('dialog');
    dialog.id='requestResultDialog';
    dialog.className='request-result-dialog';
    dialog.setAttribute('aria-labelledby','requestResultTitle');
    dialog.setAttribute('aria-describedby','requestResultText');
    dialog.innerHTML='<form method="dialog"><h2 id="requestResultTitle">Mietanfrage gespeichert</h2><div id="requestResultText"></div><button class="btn primary full-btn" autofocus>Verstanden</button></form>';
    document.body.appendChild(dialog);
  }
  const box=el('requestResultText');
  box.replaceChildren();
  const add=text=>{const p=document.createElement('p');p.textContent=text;box.appendChild(p);};
  if(isDemo){
    add('Demo-Modus: Ihre Mietanfrage wurde nur in diesem Browser gespeichert. Es wurde keine E-Mail versendet.');
  }else if(emailOk){
    add('Besten Dank für Ihre Mietanfrage. Bitte prüfen Sie Ihren E-Mail-Eingang unter '+email+'.');
    add('Keine E-Mail erhalten? Bitte kontrollieren Sie auch Ihren Spam- oder Junk-Ordner. Das gilt auch für unsere spätere Reservationsbestätigung.');
  }else{
    add('Ihre Mietanfrage wurde gespeichert. Beim automatischen E-Mailversand ist jedoch ein Fehler aufgetreten. Bitte senden Sie die Anfrage nicht nochmals.');
    add('Prüfen Sie auch Ihren Spam- oder Junk-Ordner. Falls keine Nachricht eintrifft, kontaktieren Sie uns unter info@lehmann-gt.ch.');
  }
  add('Ihre Reservation wird erst nach unserer Bestätigung verbindlich.');
  dialog.showModal();
}
